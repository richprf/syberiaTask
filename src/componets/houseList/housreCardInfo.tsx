...
const HouseCardInfo: FC<{ house: HouseProps }> = ({ house }) => {
  return (
    <div className="p-3 space-y-1 text-sm  text-gray-200">
      <h3 className="font-bold text-lg pt-4">{house.title}</h3>

      <div className="flex items-center space-x-2 text-sm text-gray-400 pt-4">
        <div className="rounded-full bg-zinc-600 flex items-center justify-center">
          <TfiMapAlt size={21} />
        </div>
      ...
      <div className="flex items-center justify-between mt-2 pt-4">
        <div>
          ...
          /
          <span className="font-bold  text-base inline text-slate-300">
            {house.price}
          </span>
        </div>
      </div>
